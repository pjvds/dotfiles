{ config, pkgs, lib, ... }: {
  ...
  launchd.user.agents.aerospace.serviceConfig.ProgramArguments = lib.mkForce [
    "/bin/sh"
    "-c"
    "PATH=/run/current-system/sw/bin:/usr/bin:/bin:/usr/sbin:/sbin /bin/wait4path /nix/store && exec ${pkgs.aerospace}/Applications/AeroSpace.app/Contents/MacOS/AeroSpace --config-path /Users/pvandesande/.config/aerospace/aerospace.toml"
  ];
